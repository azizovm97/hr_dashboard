...
def get_header_row_index(file, keywords):
...
def load_recruitment_data(file):
    file.seek(0)
    header_idx = get_header_row_index(file, ['фио кандидата', 'вакансия'])
    file.seek(0)
    ...
    cols_to_keep = ['Дата регистрации', 'Вакансия', 'Типы', 'Все подразделения', 'Тест', 'Отправлен за документами', 'Принят на работу', 'Источник подбора', 'Статус']
    existing_cols = [col for col in cols_to_keep if col in df.columns]
    df = df[existing_cols]
    
    cat_cols = ['Вакансия', 'Типы', 'Все подразделения', 'Отправлен за документами', 'Принят на работу', 'Источник подбора', 'Статус']
    for col in cat_cols:
...
def load_training_data(file):
    file.seek(0)
    header_idx = get_header_row_index(file, ['фио сотрудника', 'название курса'])
    file.seek(0)
    df = pd.read_excel(file, header=header_idx)
    df.columns = df.columns.str.replace('\n', ' ').str.replace(r'\s+', ' ', regex=True).str.strip()
    ...
    cols_to_keep = ['Дата прохождения', 'Отдел', 'Должность', 'Название курса', 'Часы обучения', 'Результат теста (%)']
    existing_cols = [col for col in cols_to_keep if col in df.columns]
    df = df[existing_cols]
...
if recruitment_file is not None:
    df_recruitment = load_recruitment_data(recruitment_file)
    ...
    if df_recruitment is not None:
        ...
        with g1:
            ...
            if 'Источник подбора' in filtered_rec.columns:
                ...
                # Перевод в текст и жесткая очистка от скрытых пробелов
                ...
                # Исключаем источники, по которым нет кандидатов
                ...
                fig_source = px.pie(
                    source_data, 
                    names='Источник', 
                    values='Количество кандидатов', 
                    hole=0.4, 
                    color_discrete_sequence=px.colors.qualitative.Pastel
                )
                
                # Выводит конкретное число кандидатов на диаграмму
        ...
        with g2:
            ...
            if 'Успешность_теста_%' in filtered_rec.columns and 'Вакансия' in filtered_rec.columns:
                ...
                avg_scores = avg_scores.sort_values(by='Успешность_теста_%', ascending=True).tail(10) # Топ-10
    ...
    if df_training is not None:
        ...
        with t_g2:
            ...
            if 'Отдел' in filtered_tr.columns and 'Часы обучения' in filtered_tr.columns:
                ...
                fig_hours = px.bar(dept_hours, x='Отдел_str', y='Часы обучения', color='Отдел_str', text_auto=True)
